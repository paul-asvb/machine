...
-- Per-system monitor settings are generated by deploy-configs.sh (answers to
-- its prompts) into machines/local.lua, which is not tracked in this repo.
-- Run deploy-configs.sh once on a fresh machine before starting Hyprland.
local machine = require("machines.local")

local monitorLeft = machine.monitorLeft
local monitorRight = machine.monitorRight
local dualMonitor = machine.dualMonitor
local flipLeftMonitor = machine.flipLeftMonitor
...
hl.monitor({
	...
	position = "auto",
	scale = 1,
	transform = (dualMonitor and flipLeftMonitor) and 1 or 0,
})
if dualMonitor then
	hl.monitor({
		output = monitorRight,
...
-- Pin workspaces to monitors: on dual-monitor setups, 1 & 2 on the left screen and the
-- rest on the right screen; on single-monitor setups, everything goes on monitorLeft.
...
if dualMonitor then
	...
	for i = 1, 10 do
		hl.workspace_rule({ workspace = tostring(i), monitor = monitorLeft })
	end
end
